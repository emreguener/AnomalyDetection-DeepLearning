...
class SimpleNet(torch.nn.Module):
    ...
    def _evaluate(self, test_data, scores, segmentations, features, labels_gt, masks_gt):
        ...
        if len(masks_gt) > 0:
            ...
            pro = metrics.compute_pro(np.squeeze(np.array(masks_gt)), 
                                            norm_segmentations)
            f1_score = metrics.compute_f1(norm_segmentations, np.squeeze(np.array(masks_gt)))
            iou_score = metrics.compute_iou(norm_segmentations, np.squeeze(np.array(masks_gt)))

        else:
            full_pixel_auroc = -1 
            pro = -1

        return auroc, full_pixel_auroc, pro, f1_score, iou_score
    ...
    def train(self, training_data, test_data):
        ...
        for i_mepoch in range(self.meta_epochs):
            ...
            scores, segmentations, features, labels_gt, masks_gt = self.predict(test_data)
            auroc, full_pixel_auroc, pro, f1_score, iou_score = self._evaluate(test_data, scores, segmentations, features, labels_gt, masks_gt)
            ...
            self.logger.logger.add_scalar("f1_score", f1_score, i_mepoch)
            self.logger.logger.add_scalar("iou_score", iou_score, i_mepoch)
            ...
            print(f"----- {i_mepoch} I-AUROC:{round(auroc, 4)}(MAX:{round(best_record[0], 4)})"
                  ...
                  f"  F1:{round(f1_score, 4)}  IoU:{round(iou_score, 4)} -----")
